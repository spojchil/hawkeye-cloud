...
检测任务的**调度核心**。接收用户提交的检测任务 → 同步持久化并立刻返回 → 异步线程池完成校验、拆分（资产 × 模板） → RocketMQ 异步投递 → 定时轮询 Redis 感知任务完成。
...
graph TD
    ...
    Cache --> MQ[RocketMQ 异步投递]
```

### TemplateCache（模板缓存）

二级缓存架构：Caffeine L1（本地） + Redis L2（分布式）
- 防缓存穿透：空值哨兵（NULL_SENTINEL）
- 防缓存击穿：逻辑过期 + 异步刷新
- 防缓存雪崩：随机过期时间
...
直接异步投递到 RocketMQ，由消费端集群竞争消费。消息携带全量执行数据，detection 消费后零外部依赖直接执行。
