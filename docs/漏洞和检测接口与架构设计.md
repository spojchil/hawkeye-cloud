...
graph TD
    ...
    Task["task-service :8005<br/>任务调度"]
    Detect["detection-service :8006+<br/>执行引擎（多实例Worker）"]

    GW --> Vul
    GW --> Task
    GW --> Detect

    Task -->|"RocketMQ<br/>task_item_topic"| Detect
    Detect -->|"Feign<br/>GET /vul/internal/{id}"| Vul
    Detect -->|"Feign<br/>GET /asset/{id}"| Asset["asset-service<br/>:8003"]

    Vul --> MySQL[("MySQL<br/>共享数据库")]
    Detect --> MySQL
    Detect --> Redis[("Redis<br/>进度计数 / 缓存")]
    Task --> Redis
```
...
graph TD
    subgraph VulService["vul-service — 知识库"]
        ...
        AsmSvc["VulTemplateAssemblyService<br/>多表 → VO / DetectDTO"]
        TplSvc --> AsmSvc
    end

    subgraph DetectService["detection-service — 执行引擎"]
        Engine["DetectionEngine<br/>编排主流程"]
        Fetcher["TemplateFetcher<br/>Caffeine L1 + Redis L2"]
        VarRes["VariableResolver<br/>占位符 → 真实值"]
        FlowEx["FlowExecutor<br/>flow → DAG执行计划"]
        HttpEx["HttpExecutor<br/>Simple / Raw HTTP"]
        Matcher["MatcherChain<br/>word → status → dsl → regex → size"]
        Extractor["ExtractorChain<br/>regex → json → kval → dsl"]
        Writer["ResultWriter<br/>本地缓冲 → 批量写DB + Redis INCR"]
...
sequenceDiagram
    ...
    participant Consumer as TaskItemConsumer
    ...
    RMQ->>Consumer: TaskItemMessage {taskId, itemId, assetId, vulId}

    Consumer->>Fetcher: fetch(vulId)
    Fetcher->>Cache: Caffeine L1?
    alt 缓存命中
        Cache-->>Fetcher: VulTemplateDetectDTO
    else L1未命中
        Fetcher->>Cache: Redis L2?
        alt L2命中
            Cache-->>Fetcher: VulTemplateDetectDTO
        else L2未命中
            Fetcher->>Vul: Feign GET /vul/internal/{id}
            Vul-->>Fetcher: VulTemplateDetectDTO
            Fetcher->>Cache: 写 Redis + Caffeine
        end
    end

    Consumer->>Asset: Feign GET /asset/{id}
    Asset-->>Consumer: Asset{url, host, port}

    Consumer->>Engine: execute(asset, template)

    Engine->>Engine: FlowExecutor.buildPlan(template)
    loop 每个步骤
        Engine->>Engine: VariableResolver.resolve(step)
        Engine->>Engine: HttpExecutor.execute(config)
        Engine->>Engine: ExtractorChain.extract(response)
        Engine->>Engine: MatcherChain.match(response)
    end

    Engine-->>Consumer: DetectionResult

    Consumer->>Writer: onResult(result)
    Writer->>RDS: INCR task:{taskId}:completed
    Writer->>Writer: buffer.add(result)
    alt buffer.size >= 500
        Writer->>DB: batch INSERT detection_result
    end
```
