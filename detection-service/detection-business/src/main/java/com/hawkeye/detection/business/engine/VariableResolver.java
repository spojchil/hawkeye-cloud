...
public class VariableResolver {
    ...
    private void initTemplateVars() {
        if (StrUtil.isNotBlank(template.getVariables())) {
            try {
                Map<String, Object> vars = JSON.parseObject(template.getVariables(),
    ...
    /** 用于从 http_requests JSON 中解析路径列表 */
    @SuppressWarnings("unchecked")
    public static List<String> parsePaths(String httpRequestsJson) {
        if (StrUtil.isBlank(httpRequestsJson)) return List.of("/");
        try {
            List<Map<String, Object>> list = JSON.parseObject(httpRequestsJson, List.class);
            if (list != null && !list.isEmpty()) {
                Map<String, Object> first = list.get(0);
                Object pathObj = first.get("path");
                if (pathObj instanceof List<?> paths) {
                    return paths.stream().map(Object::toString).toList();
                }
            }
