...
(
    `tenant_id`     BIGINT UNSIGNED AUTO_INCREMENT COMMENT '主键',
    `name`          VARCHAR(64)     NOT NULL COMMENT '租户名称',
    `contact_email` VARCHAR(128)    DEFAULT NULL COMMENT '联系邮箱',
    `status`        TINYINT         NOT NULL DEFAULT 0 COMMENT '状态: 0=启用, 1=禁用',
    `max_assets`    INT UNSIGNED    DEFAULT NULL COMMENT '资产配额上限, NULL=无限',
    `max_users`     INT UNSIGNED    DEFAULT NULL COMMENT '用户配额上限, NULL=无限',
    `max_tasks`     INT UNSIGNED    DEFAULT NULL COMMENT '任务配额上限, NULL=无限',
    `create_time`   DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`   DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `create_by`     VARCHAR(64)     NOT NULL DEFAULT '' COMMENT '创建人用户名',
    `update_by`     VARCHAR(64)     NOT NULL DEFAULT '' COMMENT '更新人用户名',
